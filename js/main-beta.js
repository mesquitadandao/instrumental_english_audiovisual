$(function(){
	...
	var $yourVocabulary = $("#your-vocabulary");
	var $yourVocabularyBody = $yourVocabulary.find('tbody');
	...
	MDTranslator.prototype.completed = function(translations){
		if(translations.length > 0){
			...
			for(var i = 0; i < lastTranslations.length; i++){
				$yourVocabularyBody.append('<tr><td data-index="'+i+'" data-key="origin"><button class="fa fa-play play-origin"></button> '+lastTranslations[i].origin.text+'</td><td  data-index="'+i+'" data-key="translation"><button class="fa fa-play play-translation"></button> '+lastTranslations[i].translation.text+'</td><tr>');
			};
			applyAudio();
			applyNextKey('origin');
